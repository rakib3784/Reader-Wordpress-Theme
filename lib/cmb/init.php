...
spl_autoload_register('reader_Meta_Box::autoload_helpers');

$meta_boxes = array();
$meta_boxes = apply_filters( 'reader_meta_boxes', $meta_boxes );
foreach ( $meta_boxes as $meta_box ) {
	$my_box = new reader_Meta_Box( $meta_box );
}

define( 'reader_META_BOX_URL', reader_Meta_Box::get_meta_box_url() );

/**
 * Create meta boxes
 */
class reader_Meta_Box {
	...
	protected static $mb_defaults = array(
		...
		'reader_styles' => true, // Include reader bundled stylesheet
		'fields'     => array(),
	);
	...
	/**
	 * Whether reader nonce has been added to the page. (oly add once)
	 * @var   bool
	 * @since 1.2.0
	 */
	...
	function __construct( $meta_box ) {
		...
		$allow_frontend = apply_filters( 'reader_allow_frontend', true, $meta_box );
		...
		$show_filters = 'reader_Meta_Box_Show_Filters';
		foreach ( get_class_methods( $show_filters ) as $filter ) {
			add_filter( 'reader_show_on', array( $show_filters, $filter ), 10, 2 );
		}

		// register our scripts and styles for reader
	...
	/**
	 * Registers scripts and styles for reader
	 * @since  1.0.0
	 */
	public function register_scripts() {
		...
		// scripts required for reader
		$scripts = array( 'jquery', 'jquery-ui-core', 'reader-datepicker', /*'media-upload', */'reader-timepicker' );
		// styles required for reader
		$styles = array();
		...
		if ( 3.5 <= $wp_version ) {
			...
			if ( ! is_admin() ) {
			   ...
			   wp_register_script( 'iris', admin_url( 'js/iris.min.js' ), array( 'jquery-ui-draggable', 'jquery-ui-slider', 'jquery-touch-punch' ), self::reader_VERSION );
		   	wp_register_script( 'wp-color-picker', admin_url( 'js/color-picker.min.js' ), array( 'iris' ), self::reader_VERSION );
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear'         => __( 'Clear' ,'reader'),
					'defaultString' => __( 'Default','reader'),
					'pick'          => __( 'Select Color' ,'reader'),
					'current'       => __( 'Current Color','reader')
				) );
			}
		} else {
		...
		wp_register_script( 'reader-datepicker', reader_META_BOX_URL . 'js/jquery.datePicker.min.js' );
		wp_register_script( 'reader-timepicker', reader_META_BOX_URL . 'js/jquery.timePicker.min.js' );
		wp_register_script( 'reader-scripts', reader_META_BOX_URL .'js/reader'. $min .'.js', $scripts, self::reader_VERSION );

		wp_enqueue_media();

		wp_localize_script( 'reader-scripts', 'reader_l10', apply_filters( 'reader_localized_data', array(
			...
			//'up_arrow'        => '[ ↑ ]&nbsp;',
			//'down_arrow'      => '&nbsp;[ ↓ ]',
			'check_toggle'    => __( 'Select / Deselect All', 'reader' ),
		) ) );

		wp_register_style( 'reader-styles', reader_META_BOX_URL . 'style'. $min .'.css', $styles );
	...
	/**
	 * Enqueues scripts and styles for reader
	 * @since  1.0.0
	 */
	public function do_scripts( $hook ) {
		// only enqueue our scripts/styles on the proper pages
		if ( $hook == 'post.php' || $hook == 'post-new.php' || $hook == 'page-new.php' || $hook == 'page.php' ) {
			wp_enqueue_script( 'reader-scripts' );

			// default is to show reader styles on post pages
			if ( $this->_meta_box['reader_styles'] )
				wp_enqueue_style( 'reader-styles' );
		}
	}
	...
	public function add_metaboxes() {
		...
		foreach ( $this->_meta_box['pages'] as $page ) {
			if ( apply_filters( 'reader_show_on', true, $this->_meta_box ) )
	...
	public function user_metabox() {
		...
		// default is to NOT show reader styles on user profile page
		if ( $this->_meta_box['reader_styles'] != false )
			wp_enqueue_style( 'reader-styles' );
	...
	public static function show_form( $meta_box, $object_id = 0, $object_type = '' ) {
		...
		echo "\n<!-- Begin reader Fields -->\n";
		do_action( 'reader_before_table', $meta_box, $object_id, $object_type );
		echo '<table class="form-table reader_metabox">';
		...
		foreach ( $meta_box['fields'] as $field_args ) {
			...
			if ( 'group' == $field_args['type'] ) {
				...
				$field = new reader_Meta_Box_field( $field_args );
				$field->render_field();
			}
		}
		echo '</table>';
		do_action( 'reader_after_table', $meta_box, $object_id, $object_type );
		echo "\n<!-- End reader Fields -->\n";
	...
	public static function render_group( $args ) {
		...
		$args['count']   = 0;
		$field_group     = new reader_Meta_Box_field( $args );
		...
		if ( $desc || $label ) {
			echo '<tr><th>';
				if ( $label )
					echo '<h2 class="reader-group-name">'. $label .'</h2>';
				if ( $desc )
					echo '<p class="reader_metabox_description">'. $desc .'</p>';
			echo '</th></tr>';
		}
	...
	public static function render_group_row( $field_group, $remove_disabled ) {
		...
		<tr class="repeatable-grouping" data-iterator="'. $field_group->count() .'">
			<td>
				<table class="reader-nested-table" style="width: 100%;">';
				if ( $field_group->options( 'group_title' ) ) {
					echo '
					<tr class="reader-group-title">
						<th colspan="2">
				...
				foreach ( array_values( $field_group->args( 'fields' ) ) as $field_args ) {
					...
					$field = new reader_Meta_Box_field( $field_args, $field_group );
					$field->render_field();
				}
				echo '
					<tr>
	...
	public static function save_fields( $meta_box, $object_id, $object_type = '' ) {
		...
		if ( ! apply_filters( 'reader_show_on', true, $meta_box ) )
			return;
		...
		do_action( "reader_save_{$object_type}_fields", $object_id, $meta_box['id'], self::$updated, $meta_box );
	 ...
	 * @since  1.0.1
	 * @return string URL to reader resources
	 */
	public static function get_meta_box_url() {
		...
		if ( strtoupper( substr( PHP_OS, 0, 3 ) ) === 'WIN' ) {
			...
			$reader_url = str_replace( DIRECTORY_SEPARATOR, '/', $content_url );

		} else {
		  $reader_url = str_replace(
		...
		$reader_url = set_url_scheme( $reader_url );

		return trailingslashit( apply_filters('reader_meta_box_url', $reader_url ) );
	}
	 ...
	 * @uses apply_filters() Calls 'reader_override_option_get_$option_key' hook to allow
	...
	public static function _get_option( $option_key, $default = false ) {
		...
		$test_get = apply_filters( "reader_override_option_get_$option_key", 'reader_no_override_option_get', $default );

		if ( $test_get !== 'reader_no_override_option_get' )
			return $test_get;
	 ...
	 * @uses apply_filters() Calls 'reader_override_option_save_$option_key' hook to allow
	...
	public static function save_option( $option_key ) {
		...
		$test_save = apply_filters( "reader_override_option_save_$option_key", 'reader_no_override_option_save', $to_save );

		if ( $test_save !== 'reader_no_override_option_save' )
			return $test_save;
...
// Handle oembed Ajax
add_action( 'wp_ajax_reader_oembed_handler', array( 'reader_Meta_Box_ajax', 'oembed_handler' ) );
add_action( 'wp_ajax_nopriv_reader_oembed_handler', array( 'reader_Meta_Box_ajax', 'oembed_handler' ) );

/**
 * A helper function to get an option from a reader options array
 * @since  1.0.1
 * @param  string  $option_key Option key
 * @param  string  $field_id   Option array field key
 * @return array               Options array or specific field
 */
function reader_get_option( $option_key, $field_id = '' ) {
	return reader_Meta_Box::get_option( $option_key, $field_id );
}

/**
 * Get a reader field object.
 * @since  1.1.0
 * @param  array  $field_args  Field arguments
 * @param  int    $object_id   Object ID
 * @param  string $object_type Type of object being saved. (e.g., post, user, or comment)
 * @return object              reader_Meta_Box_field object
 */
function reader_get_field( $field_args, $object_id = 0, $object_type = 'post' ) {
	// Default to the loop post ID
	$object_id = $object_id ? $object_id : get_the_ID();
	reader_Meta_Box::set_object_id( $object_id );
	reader_Meta_Box::set_object_type( $object_type );
	// Send back field object
	return new reader_Meta_Box_field( $field_args );
}
 ...
 */
function reader_get_field_value( $field_args, $object_id = 0, $object_type = 'post' ) {
	$field = reader_get_field( $field_args, $object_id, $object_type );
 ...
 * @param int   $object_id  Object ID
 */
function reader_print_metaboxes( $meta_boxes, $object_id ) {
	foreach ( (array) $meta_boxes as $meta_box ) {
		reader_print_metabox( $meta_box, $object_id );
	}
}
 ...
 * @param int   $object_id Object ID
 */
function reader_print_metabox( $meta_box, $object_id ) {
	$reader = new reader_Meta_Box( $meta_box );
	if ( $reader ) {

		reader_Meta_Box::set_object_id( $object_id );

		if ( ! wp_script_is( 'reader-scripts', 'registered' ) )
			$reader->register_scripts();

		wp_enqueue_script( 'reader-scripts' );

		// default is to show reader styles
		if ( $meta_box['reader_styles'] != false )
			wp_enqueue_style( 'reader-styles' );

		reader_Meta_Box::show_form( $meta_box );
	}
 ...
 * @param int   $object_id Object ID
 */
function reader_save_metabox_fields( $meta_box, $object_id ) {
	reader_Meta_Box::save_fields( $meta_box, $object_id );
}
 ...
 * @return string             reader html form markup
 */
function reader_metabox_form( $meta_box, $object_id, $echo = true ) {

	$meta_box = reader_Meta_Box::set_mb_defaults( $meta_box );
	...
	reader_Meta_Box::set_object_type( reader_Meta_Box::set_mb_type( $meta_box ) );
	...
	if (
		// check nonce
		isset( $_POST['submit-reader'], $_POST['object_id'], $_POST['wp_meta_box_nonce'] )
		&& wp_verify_nonce( $_POST['wp_meta_box_nonce'], reader_Meta_Box::nonce() )
		&& $_POST['object_id'] == $object_id
	)
		reader_save_metabox_fields( $meta_box, $object_id );
	...
	$form_format = apply_filters( 'reader_frontend_form_format', '<form class="reader-form" method="post" id="%s" enctype="multipart/form-data" encoding="multipart/form-data"><input type="hidden" name="object_id" value="%s">%s<input type="submit" name="submit-reader" value="%s" class="button-primary"></form>', $object_id, $meta_box, $form );

	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, __( 'Save', 'reader' ) );
